//! The terminal player: argument parsing, the terminal, and the event loop.

use crate::{app, cache, config, library, media, ui, worker};
...
USAGE
    {name} [FOLDER | tuneterm://SERVER] [OPTIONS]
    {name} serve | push | ls | mv | rm ...

ARGS
    FOLDER            Music folder to browse. Defaults to the Apple Music
                      library if present, otherwise ~/Music. A tuneterm://
                      address browses a server instead, the same way.
...
fn scan_report(root: &Path) -> io::Result<()> {
    ...
    for track in tracks.iter().take(8) {
        let name = track
            ...
            .unwrap_or_default();
        let cover = match &track.art_url {
            Some(url) if crate::remote::is_remote(url) => {
                crate::remote::cover(url).and_then(|bytes| image::load_from_memory(&bytes).ok())
            }
            _ => library::load_cover(&track.path),
        };
        match cover {
...
fn on_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char('+') | KeyCode::Char('=') => app.nudge_volume(0.02),
        KeyCode::Char('-') => app.nudge_volume(-0.02),
